"""
World Bank Indicators - External Debt -- TrendSpider store indicator by TrendSpider.

Registered as "world_bank_indicators_external_debt_TS". Translated line-for-line from the script's
published JavaScript (https://trendspider.com/trading-tools-store/indicators/6a6299-world-bank-indicators-external-debt/)
...
def script(G):
    ...
    validIndicatorIds = J.JSArray(["BM.GSR.TOTL.CD", "BN.CAB.XOKA.CD", "BX.GSR.TOTL.CD", "BX.KLT.DINV.CD.WD", "BX.PEF.TOTL.CD.WD", "BX.TRF.PWKR.CD.DT", "FI.RES.TOTL.CD", "FI.RES.TOTL.MO", "NY.GNP.MKTP.CD"])
    data = J.get(G_request, "http")(J.template("https://api.worldbank.org/v2/topic/20/indicator?per_page=1000&format=json"))
    ...
    G_describe_indicator(J.template("World Bank Indicators - External Debt"), "lower", J.obj(("citation", indicatorNote), ("shortName", J.template("WBI Debt"))))
...
register_store_indicator(
    script,
    name='world_bank_indicators_external_debt_TS',
    title='World Bank Indicators - External Debt',
    developer='TrendSpider',
    url='https://trendspider.com/trading-tools-store/indicators/6a6299-world-bank-indicators-external-debt/',
    position='lower',
    inputs=[{'id': 'indicator', 'title': 'Indicator', 'type': 'select_wide', 'default': 'Indicator BX.TRF.PWKR.CD.DT', 'options': ['Indicator BX.TRF.PWKR.CD.DT', 'Indicator BX.KLT.DINV.CD.WD', 'Indicator BX.PEF.TOTL.CD.WD', 'Indicator FI.RES.TOTL.MO', 'Indicator FI.RES.TOTL.CD', 'Indicator BM.GSR.TOTL.CD', 'Indicator BN.CAB.XOKA.CD', 'Indicator BX.GSR.TOTL.CD', 'Indicator NY.GNP.MKTP.CD']}, {'id': 'country', 'title': 'Country', 'type': 'select_wide', 'default': 'United States', 'options': ['Australia', 'Canada', 'China', 'European Union', 'Japan', 'Mexico', 'United Kingdom', 'United States', 'World']}],
    outputs=['line'],
    signals=[],
    requires=['http'],
    parity='exact',
