"""
Smoothed Heikin Ashi -- TrendSpider store indicator by TrendSpider Team.

Registered as "smoothed_heikin_ashi_TS". Translated line-for-line from the script's
published JavaScript (https://trendspider.com/trading-tools-store/indicators/smoothed-heikin-ashi/)
...
def script(G):
    ...
    G_time = G["time"]
    G_describe_indicator("Smoothed HA")
    ...
    haSmoothHighSeries = J.get(G_indicators, afterMAType)(haHighSeries, afterSmoothLength)
    haSmoothLowSeries = J.get(G_indicators, afterMAType)(haLowSeries, afterSmoothLength)
    haSmoothCloseSeries = J.get(G_indicators, afterMAType)(haCloseSeries, afterSmoothLength)
    def _f1(haOpen_2=J.undefined, haHigh_2=J.undefined, haLow_2=J.undefined, haClose_2=J.undefined, *_args):
        return J.obj(("high", haHigh_2), ("low", haLow_2), ("q1", haClose_2), ("median", haOpen_2), ("q3", haOpen_2), ("fillColor", ("green" if J.gt(haClose_2, haOpen_2) else "red")))
    result = G_for_every(haSmoothOpenSeries, haSmoothHighSeries, haSmoothLowSeries, haSmoothCloseSeries, _f1)
    def _f2(record=J.undefined, *_args):
        if (not J.truthy(record)):
            return "gray"
        return ("green" if J.gt(J.get(record, "q1"), J.get(record, "q3")) else "red")
    color = J.get(result, "map")(_f2)
    G_paint(result, J.obj(("style", "boxplot"), ("color", color)))


register_store_indicator(
    script,
    name='smoothed_heikin_ashi_TS',
    title='Smoothed Heikin Ashi',
    developer='TrendSpider Team',
    url='https://trendspider.com/trading-tools-store/indicators/smoothed-heikin-ashi/',
    position='price',
